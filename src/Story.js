...
const Title = styled.span`padding: 10px;`;
					...
					<Wrapper>
						<Container>
							...
							<Content>
								...
								<Caption>{caption.map ? caption.map((text) => <div>{text}</div>) : caption}</Caption>
								{captionLink && <Link href={captionLink.href}>{captionLink.text}</Link>}
								{desc && (
									<React.Fragment>
										<Line />
										<Text>
