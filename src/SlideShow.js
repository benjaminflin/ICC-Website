...
class SlideShow extends React.Component {
	...
	async getSlides() {
		const slides = await fetch(`/assets/slides-${this.props.lang}.json`).then((res) => res.json());
		const disclaimer = await fetch(`/assets/disclaimer-${this.props.lang}.txt`).then((res) => res.text());
			...
			<Wrapper ref={this.ref}>
				{this.state.slides.map((slide, index) => {
					...
					switch (slide.layout) {
						...
						case 'ordered-list':
							return <ListSlide key={index} active={active} {...slide} ordered />;
						case 'unordered-list':
							return <ListSlide key={index} active={active} {...slide} />;
						case 'photographer':
							return <PhotographerSlide key={index} active={active} {...slide} />;
						case 'links':
							return <LinksSlide key={index} active={active} {...slide} />;
						case 'intro':
							return <IntroSlide key={index} active={active} {...slide} />;
						default:
							return <BlankSlide key={index} index={index} active={active} title="Invalid Slide" />;
					}
				})}
			</Wrapper>
		);
